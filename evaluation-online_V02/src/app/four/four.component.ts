...
export class FourComponent implements OnInit {
  ...
  public NoFilm = {
    id: "0",
    rec: ['None'],
    image: "../../assets/online-evaluations/listsort.jpg",
    title: "Arraste aqui",
    sinopse: "",
  }
  public listSelectedMovies = [this.NoFilm]
  ...
  ngOnInit() {
    ...
    if((this.intr % 2) == 0){
      this.listMovies = this.listAMovies;
    }else{
      this.listMovies = this.listBMovies;
    }
    this.listMovies = this.shuffle(this.listMovies);
  }
  ...
  removeFilm(array, element) {
      return array.filter(e => e !== element);
  }

  sendEvaluation(){
    /*let listMoviesFinal = [];
    this.listMovies.forEach(element => {
      listMoviesFinal.push({
        id: element.id
      })
    });
    console.log(this.removeFilm(this.listSelectedMovies, this.NoFilm));*/
  
    this.time = new Date().getTime() - this.time;
  
    this.angFireData.database.ref('/Users/' + this.user +'/PageFour').set({
      SelectedMovies: this.listSelectedMovies,
      time: this.time
    }).then((res) => {
      this.router.navigate(['/userinfo']);
    }).catch((err) => {
      console.log("Erro ao inserir lista de filmes selecionados: " + err);
    })
  }
  ...
  sendNoEvaluation(content){
    ...
    this.modalRef.result.then((result) => {
      if(result == "CONFIRM"){
        ...
        this.angFireData.database.ref('/Users/' + this.user +'/PageFour').set({
          SelectedMovies: "Nenhum filme escolhido",
          time: this.time
        }).then((res) => {
          this.router.navigate(['/userinfo']);
        }).catch((err) => {
          console.log("Erro ao enviar nenhum filme: " + err);
        })
