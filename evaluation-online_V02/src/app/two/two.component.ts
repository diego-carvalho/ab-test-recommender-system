...
export class TwoComponent implements OnInit {
  ...
  @ViewChild('contextModel') contextModelContent;
  ...
  sendEvaluation(){
    if(this.movie){
      ...
      let movieObj: any = this.getTitleById(this.movie);
  ...
  getTitleById(id): string{
    ...
    let pos: number = id[1];
    if(id[0] == "R"){
      movieObj = this.carouselRecItems[pos - 1];
    }else if(id[0] == "M"){
      movieObj = this.carouselMaxItems[pos - 1];
    }else if(id[0] == "N"){
      movieObj = this.carouselNiItems[pos - 1];
    }else if(id[0] == "B"){
      movieObj = this.carouselBestItems[pos - 1];
    }else if(id[0] == "P"){
      movieObj = this.carouselPopularItems[pos - 1];
    }
    return movieObj;
  }
  ...
  openContextModel(){
    this.modalService.open(this.contextModelContent).result.then((result) => {
      if(result == "CONFIRM"){
