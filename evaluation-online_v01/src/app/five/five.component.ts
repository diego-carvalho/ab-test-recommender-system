...
export class FiveComponent implements OnInit {
    ...
    constructor(private modalService: NgbModal, private router: Router, private angFireData: AngularFireDatabase,
                private objService: ObjPageFiveService) {
    ...
    checkEquivality(obj, opc){
      ...
      if(opc){
        ...
        let objSix = obj;
        objSix.modal = false;
        this.listOfObjects.push(objSix)
      }else{
        let index = -1;
        ...
        index = -1;
        let cont = 0;
        for(let i of this.listOfObjects){
          if(i.movieId == obj.movieId){
            index = cont;
            break;
          }
      ...
      console.log("OBJ: ", this.listOfObjects);
    }
    ...
    sendEvaluation(){
      ...
      this.angFireData.database.ref('/Users/' + this.user +'/PageFive').set({
        ...
        if(this.listOfObjects.length > 0){
          this.objService.setObj(this.listOfObjects);
          this.objService.setAllIds(this.selectItems);
